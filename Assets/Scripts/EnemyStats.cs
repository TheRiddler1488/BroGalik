...
public class EnemyStats : MonoBehaviour
{
    ...
    void Die()
    {
        if (gameManager != null)
        {
            gameManager.souls += gameManager.soulsPerKill; // Используем значение из GameManager
            gameManager.AddExperience(gameManager.experiencePerKill); // Используем значение из GameManager
            if (UIManager.Instance != null) UIManager.Instance.UpdateUI();
        }
        if (pool != null)
        {
